  ...
  try {
    ...
    if (taskStatus === 'Completed') {
      ...
      if (videos.length > 0) {
        ...
        return { done: true, url: `${playerUrl}/#${videoId}`, error: null };
      }
...
export async function POST(req: NextRequest) {
  ...
  if (!tasks || tasks.length === 0) {
    return NextResponse.json({ ok: true, processed: 0, checked: 0 });
  }
  ...
  for (const task of tasks) {
    ...
    if (!updateTaskError) processed++;
  }

  return NextResponse.json({ ok: true, processed, checked: tasks.length });
}
